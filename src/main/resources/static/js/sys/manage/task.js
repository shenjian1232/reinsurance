    ...
    var app = new Vue({
        ...
        data: {
            ...
            taskFormRules: {
                ...
                cronExpression: [
                    {required: true, message: '请输入定时计划', trigger: 'blur'}
                ]
            },
            filterText: '',
            //data2: menuTree,
        ...
        methods: {
            ...
            loadData: function (criteria, currentPage, pageSize) {
                ...
                this.$http.get("get_tasks_json", criteria).then(function (res) {
            ...
            handleRefresh: function () {
                window.location.href = "/api/a/manage/task/index";
            },

            // 启动所有状态为1的定时任务
            runAllTask: function () {
               // var id = row.id;
                this.$confirm('确定要启动服务?', '提示', {
            ...
            handleChangetStatus: function (index, row) {
                var id = row.id;
                var jobStatus = row.jobStatus == 1 ? 0 : 1;
                this.$confirm('确定修改该条记录的状态?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    changeStatus(this, id, jobStatus);
            }).catch(() => {
                    this.$message({
                    showClose: true,
            ...
            handleAdd: function () {
                ...
                this.taskForm.jobDescription = '';
                this.taskForm.jobStatus = 0;
                this.taskForm.cronExpression = '';
                this.taskForm.milliSeconds = '';
                this.taskForm.repeatCount = '';
            ...
            handleEdit: function (index, row) {
                ...
                this.taskForm.jobName = row.jobName;
                this.taskForm.jobDescription = row.jobDescription;
                this.taskForm.jobStatus = row.jobStatus;
                this.taskForm.cronExpression = row.cronExpression;
                this.taskForm.milliSeconds = row.milliSeconds;
                this.taskForm.repeatCount = row.repeatCount;
                this.taskForm.startDate = row.startDate;
                this.taskForm.endDate = row.endDate;

                var _this = this;
                // Vue.nextTick(function () {
                //     _this.$refs.editTree.setCheckedKeys(_this.taskForm.menuIds);
                // })
            },
            ...
            editSubmit: function () {
                var _this = this;
                //this.taskForm.menuIds = this.$refs.editTree.getCheckedKeys();
                this.$refs.taskForm.validate((valid) => {
                    if (valid) {
...
function changeStatus(_this, id, jobStatus) {
    $.post("changeStatus", {id: id, jobStatus: jobStatus}, function (a, b) {
        if (a == true) {
            _this.$notify({
                title: '成功',
...
// 启动所有状态为1的批处理
function runAllTaskJob (_this) {
    //window.location.href = "/api/a/manage/task/addAllJob";
    $.post("addAllJob",  function (a, b) {
        if (a == true) {
            _this.$notify({
                title: '成功',
                message: '恭喜您, 服务启动成功成功',
                type: 'success'
            });
            _this.loadData(_this.criteria, _this.currentPage, _this.pageSize);
        } else {
